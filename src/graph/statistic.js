...
export function testSoundness(jsonfile, f) {
    var _a, _b;
    ...
    for (const m of f.a.moduleInfos.values())
        if ((_a = m.node) === null || _a === void 0 ? void 0 : _a.loc) // TODO: m.node.loc may be empty with --ignore-dependencies
    ...
    for (const g of f.a.functionInfos.values())
        if (!((_b = g.node) === null || _b === void 0 ? void 0 : _b.loc) || "nodeIndex" in g.node.loc)
    ...
    const dyn = JSON.parse(readFileSync(jsonfile, "utf8"));
    ...
    function findRepresentativeLocation(loc) {
        const c = loc.indexOf(":");
    ...
    const dynamicFunctionLocs = new Map();
    ...
    const comp = ([, loc1], [, loc2]) => loc1 < loc2 ? -1 : loc1 > loc2 ? 1 : 0;
