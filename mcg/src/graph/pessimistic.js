...
function addOneShotEdges(ast, fg) {
    // console.log(ast);
    ast.functions.forEach(function (fn) {
        try {
        ...
        // 表示函数是一次性闭包
        // console.log(fn);
        if ( childProp && childProp === 'callee' && parent &&
    ...
    ast.calls.forEach(function (call) {

        if (!call.attr.oneshot)
            // 在流图 fg 中添加从每个参数顶点（flowgraph.argVertex(call, i)）到未知顶点（flowgraph.unknownVertex()）的边
            for (var i = 0; i <= call.arguments.length; ++i) {
                if (call.arguments[i] &&
                    ((call.arguments[i].type === 'FunctionExpression' ||
                        call.arguments[i].type === 'ArrowFunctionExpression' ||
                        call.arguments[i].type === 'FunctionDeclaration'))

                ) {
                    try {
                        if (tool.nativeCalls().indexOf(call.attr.path.split(' ')[call.attr.path.split(' ').length - 1]) != -1) {
                            fg.addEdge(flowgraph.nativeArgVertex(call, i),
                                flowgraph.vertexFor(call.callee,
                                    call.attr.enclosingFile + '<' +
                                    call.loc.start.line + ','
                ...
                else if (call.arguments[i] && call.arguments[i].type === 'Identifier' &&
                    tool.nativeCalls().indexOf(call.attr.path.split(' ')[call.attr.path.split(' ').length - 1]) != -1) {
                    fg.addEdge(flowgraph.nativeArgVertex(call, i),
                        flowgraph.vertexFor(call.callee,
                            call.attr.enclosingFile + '<' +
                            call.loc.start.line + ','
