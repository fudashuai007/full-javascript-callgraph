...
var stubUtils_js_1 = require("./stubUtils.js");
...
var depList;
if (callgraphpath) {
    var targets = (0, stubUtils_js_1.getTargetsFromACG)(callgraphpath);
    functions = targets.map(stubUtils_js_1.buildHappyName);
    listedFiles = targets.map(stubUtils_js_1.getFileName);
    noCG = false;
}
var zipFiles = false; // Currently don't ever do this.
if (removeFunsPath) {
    removeFuns = (0, stubUtils_js_1.getTargetsFromACG)(removeFunsPath).map(stubUtils_js_1.buildHappyName);
}
if (uncoveredMode) {
    var targets = (0, stubUtils_js_1.getTargetsFromCoverageReport)(coverageReportPath);
    functions = targets.map(stubUtils_js_1.buildHappyName);
    var all_listedFiles = targets.map(stubUtils_js_1.getFileName);
...
if (bundlerMode != "no") {
    if (bundlerMode == "bundle_and_stub" || bundlerMode == "only_bundle") {
        ...
        (0, stubUtils_js_1.generateBundlerConfig)(path.resolve(filename));
